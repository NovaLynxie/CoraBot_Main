...
router.get('/:guildID', checkAuth, async (req, res) => {
  ...
  const data = {
    moderation: await client.data.moderation.get(guild),
    voice: await client.data.voice.get(guild),
    trackers: await client.data.trackers.get(guild)
  };
...
router.get('/:guildID/ban/:userID', checkAuth, async (req, res) => {
  ...
  if (req.params.userID === client.user.id || req.params.userID === req.user.id) {
    req.flash('warning', 'You cannot ban yourself or the bot from here! If you want to remove the bot, please use the LEAVE button.');
