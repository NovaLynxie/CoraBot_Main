...
async function syncEconomyUserData(users) {
  logger.debug(`Syncing economy data for ${users.size} users.`);
  ...
  for (const userID of userIDs) {
    ...
    if (!econUser) {
      ...
      logger.verbose(`UserID:${userID} already exists!`);
    };
    logger.verbose('Saving economy user data...');
    await guildDataStore.economy.users.set(userID, econUser);
    logger.verbose(`Saved User:${userID} successfully!`);
  };
  logger.debug(`Finished syncing economy user data!`);
};
async function readEconomyUserData(user, route) {
  logger.verbose(`Fetching user economy.${route} data for ${user.name} (ID:${user.id})`);
  const res = await guildDataStore.economy[route].get(user.id);
  return res;
};
async function saveEconomyUserData(user, route, data) {
  logger.verbose(`Updating user economy.${route} data for ${user.name} (ID:${user.id})`);
  await guildDataStore.economy[route].set(user.id, data);
};
