...
async function syncEconomyUserData(users) {
  ...
  for (const userID of userIDs) {
    ...
    if (!econUser) {
      ...
      logger.verbose(`UserID:${userID} already exists!`); continue;
    };
...
async function readEconomyData(req, route) {
  logger.verbose(`Fetching user economy.${route} data for ${req.name} (ID:${req.id})`);
  const res = await guildDataStore.economy[route].get(req.id);
  return res;
};
async function saveEconomyData(req, route, data) {
  logger.verbose(`Updating user economy.${route} data for ${req.name} (ID:${req.id})`);
  await guildDataStore.economy[route].set(req.id, data);
};
