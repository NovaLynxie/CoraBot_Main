...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    function generateImageEmbed(data) {
      let entry, image, random = Math.floor(Math.random() * 10);
      logger.debug(JSON.stringify(data, null, 2));
      const imageEmbed = new MessageEmbed();
      switch (source) {
        case 'e621':
        case 'e926':
          entry = data.posts[random]; image = { url: entry.file.url };
          break;
        case 'derpibooru':
        case 'gelbooru':
          entry = data.images[random]; image = { url: entry.view_url };
          break;
        default:
          image = null;
          throw new Error(`Unsupported or invalid source ${source}!`);
      }; 
      logger.debug(`image:${JSON.stringify(image, null, 2)}`);
