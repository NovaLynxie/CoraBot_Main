...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    function generateImageEmbed(data) {
      let entry, image, tags = '';
      const randomizer = (length) => Math.floor(Math.random() * parseInt(length));
      ...
      switch (source) {
        ...
        case 'e926':
          ...
          image = (entry) ? { url: entry.file.url } : null;
          break;
        case 'derpibooru':
        case 'furbooru':
        case 'gelbooru':
          entry = data.images[randomizer(data.images.length)];
          image = (entry) ? { url: entry.view_url } : null;
          break;
        default:
          image = null;
