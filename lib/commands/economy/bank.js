...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    let mode, economyEmbed = new MessageEmbed().setColor('#e39d5f');
    const econUser = await client.data.economy.get(target || member.user, 'users');
      ...
      case 'view':
        if (!econUser) {
          economyEmbed
            ...
            .setDescription(`No accounts was found for ${target || member}!`);
        } else {
          economyEmbed
            .setTitle('Economy - Account Viewer')
            .setDescription(`
            Name: ${target || member}
            Funds: ${econUser.balance >= 0 ?  econUser.balance : 'N/A'}
            `);
        }; break;
      case 'transfer':
