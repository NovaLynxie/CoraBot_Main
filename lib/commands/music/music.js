...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    async function sourceParser(url) {
      ...
      await client.data.voice.set(voiceData, guild);
    ...
    if (collector) {
      collector.on('collect', async interact => {
        ...
        try {
          ...
          }; await client.data.voice.set(voiceData, interact.guild);
        } catch (err) {
      ...
      collector.on('end', async collected => {
        ...
        await client.data.voice.set(voiceData, interaction.guild);
      });
    } else return;
  },
};